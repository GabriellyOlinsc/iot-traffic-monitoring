...
### Opção 1 — Tudo de uma vez (recomendado para testes)

```bash
python run_all.py
```

Os dispositivos sobem na ordem correta com um intervalo entre cada um.
Para parar tudo: `Ctrl+C`

Também é possível ajustar o delay de inicialização entre dispositivos:
...
> ⚠️ A ordem importa. O Cloud Server e o Smart Gateway precisam estar de pé antes dos sensores e atuadores.
...
| Smart Gateway    | Ciano      |
| Cloud Server     | Azul       |
| Inductive Loop   | Verde      |
| Radar            | Amarelo    |
| Incident Sensor  | Vermelho   |
| LED Panel        | Magenta    |
| Traffic Light    | Laranja    |
| Run All          | Branco     |
